...
document.addEventListener('DOMContentLoaded', () => {
  ...
  TabsSliders();
    ...
    const spheresTabsInit = () => {
      const spheresButtons = document.querySelectorAll('.tabs__nav-btn,.spheres__item--mobile');
      let spheresBodys = document.querySelectorAll('.tabs__panel');
      ...
      if (spheresButtons.length > 0 && spheresBodys.length > 0 && ajaxUrlItem){
        ...
        spheresButtons.forEach(spheresButton => {
          spheresButton.addEventListener('click', () => {
            ...
            if (isMobile()){
              spheresButton.scrollIntoView();
            }
            let ajaxId = spheresButton.dataset.tab;
            let cacheBx = '';
          
            if (currentURL.indexOf('clear_cache=Y') !== -1) {
              // Если содержит, добавляем его к переменной
              cacheBx += '&clear_cache=Y';
            
            }
            if (!tabItem.classList.contains(ajaxSuccessClass)) {
              fetchData(`${ajaxUrl}?id=${ajaxId+cacheBx}`, response => {
                ...
                if (newTab) {
                  let currentTab;
                  spheresBodys.forEach(element => {
                    // Проверяем значение атрибута data-tab-item у текущего элемента
                    if (element.dataset.tabItem === spheresButton.dataset.tab) {
                        // Найден элемент, можно с ним работать
                      currentTab = element
                        // Выход из цикла, если нужный элемент найден
                        return;
                    }
                });
                  currentTab.innerHTML = newTab.innerHTML;
                    
                  currentTab.classList.add('tabs__panel--active', ajaxSuccessClass);
                  spheresButton.classList.add('tabs__nav-btn--active');
                  ajaxTabs();
                  initModals()
                  TabsSliders();
               
                  // swiperSpheres.update();
                } else {
              ...
              spheresButton.classList.add('tabs__nav-btn--active');
              spheresBodys = document.querySelectorAll('.tabs__panel');
              tabItem.classList.add('tabs__panel--active');
            }
          });
        });
      }
      ...
      tabs.forEach(tab => {
        ...
        const removeAllTabs = () => {
          tabHeaders.forEach(tabHeader => {
            ...
            tabHeader.classList.add('js-disabled')
          });
        ...
        tabHeaders.forEach((tabHeader, index) => {
          tabHeader.addEventListener('click', () => {
            ...
            let ajaxId = tabHeader.dataset.cstTabsButton;
    
            let tabBody = tab.querySelector(`[data-cst-tabs-body="${ajaxId}"]`);
            ...
            if (!tabBody.classList.contains(ajaxSuccessClass)) {
              fetchData(`${ajaxUrl}?id=${ajaxId+cacheBx}`, response => {
                ...
                const tempElement = document.createElement('div');
                tempElement.innerHTML = response;
                const currentTab = tempElement.querySelector(`[data-cst-tabs-body="${ajaxId}"]`);
              
                // const newTabBody = currentTab.querySelector(`[data-cst-tabs-body="${tabHeader.dataset.cstTabsButton}"]`);
                if (currentTab) {
                  tabBody.innerHTML = currentTab.innerHTML;
                  initGallerySlider();
                  refreshFsLightbox();
                  initModals()
                  ...
                  tabHeaders.forEach(tabHeader => {
                    ...
                    tabHeader.classList.remove('js-disabled')
                  });
                } else {
  ...
  const validatonForms = () => {
    ...
    if (document.querySelector('#CALLBACK')) {
      validateForms('#callback', rules1, afterForm);
    }
    if (document.querySelector('#RATE')) {
      validateForms('#RATE', rules1, afterForm);
    }
    ...
    MicroModal.init({
      ...
      onClose: modal => {
        ...
        forms.forEach(form => {
          ...
          // Убираем состояния ошибок валидации (если библиотека не имеет метода сброса)
          form.querySelectorAll('.just-validate-error-label').forEach(errorMessage => {
            errorMessage.remove(); // удаляем сообщение об ошибке
          })
          form.querySelectorAll('.just-validate-error-field').forEach(errorField => {
            errorField.classList.remove('just-validate-error-field');
          });
  
          // Деактивируем кнопку отправки формы, если это требуется логикой
  ...
  if (document.querySelector('.audit-examples__slider')) {
    var auditExamplesSlider = new Swiper(".audit-examples__slider", {
      spaceBetween: 10,
      slidesPerView: "auto",
      loop: true,
      navigation: {
        nextEl: ".audit-examples__arrow--next",
        prevEl: ".audit-examples__arrow--prev",
      },
    });
  }
});
